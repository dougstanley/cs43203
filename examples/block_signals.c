...
int main(int argc, char *argv[]){
    ...
    /* Now we set up the mask for our process.
     * The previous mask was what signals to ignore while
     * our code is inside the signal handler function.
     * This mask is the set of signals to ignore for the
     * process itself. So since we're setting up handlers for
     * SIGINT and SIGUSR1, we'll ignore all signals besides
     * those two.
     */
    sigfillset( &proccess_mask );
    sigdelset( &proccess_mask, SIGINT);
    sigdelset( &proccess_mask, SIGUSR1);
